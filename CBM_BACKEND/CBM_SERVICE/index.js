...
setInterval(function(){ 
    valuesReady()
    // console.log(settings.plc_refreshrate)
...
// },3000)
    ...
    app.get("/onehourdata", async (req, res) => {
        ...
        try {
            var sta={}    
            const results =  await sequelize.query(`SELECT DISTINCT station FROM actual_data where time_stamp > now() - interval 30 minute `);
            const re = await results[0]
            for await (const data of re) {
                const s = await new Date().getTime()
                const select =  await sequelize.query(`select * from actual_data where station='${data.station}' AND type='type' AND time_stamp > now() - interval 30 minute `);
                const e = await new Date().getTime()

                // console.log(e-s);

                for await (const ele of select[0]) {
                    // console.log(data?.station, select[0].length);
                    if(sta[ele?.station]){
