...
var noOfTools=stn_setting.no_of_tools 
    ...
    globalVariableUpdate()
        ...
        async function exportData(token,plcTagValues,tool,currentlifecycle,m_code,stn){
                try {
                    // console.log(m_code);
                        var shiftId="S"+plcTagValues.shift;
                        var hours = new Date().getHours();
                        var ampm = (hours >= 12) ? "PM" : "AM";
                        ...
                        // console.log("Teal Response For ToolLife :"+res.data ,"TOOL ID :",tool,"CURRENT LIFE CYCLE :",currentlifecycle);   
                    ...
                    datalogger.dataLog("Tool Life exportData func",error);
                    // console.error("error statement from export data "+error);
